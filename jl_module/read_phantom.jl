...
function _read_capture_pattern(fp)
    ...
    if (i1 != 60769)
        ...
        # retry assert
        if i1 != 60769
            error("Capture pattern error. i1 mismatch. Is this a Phantom data file?")
        end
    end
    ...
    if (i3 != 690706)
        error("Capture pattern error. i3 mismatch. Is this a Phantom data file?")
    end
    ...
    # assert tags equal
    if (start_tag != end_tag)
       error("Capture pattern error. Fortran tags mismatch. Is this a Phantom data file?")
    end
...
function _read_global_header_block(fp,dtype)
    ...
    if nvars > 0
        ...
        keys = [strip(String(keys_data[i:i+15])) for i in 1:16:length(keys_data)]
...
function read_phantom(filename::String, separate_types::String = "sinks", ignore_inactive::Bool = true)
    ...
    println("Loaded filepath: ",filename)
    ...
    open(filename,"r") do fp
        ...
        if (separate_types == "all") && (hasproperty(df, :"itype")) && (length(unique(df.itype))>1)
            ...
            for group in groupby(df,:"itype")
                ...
                group_clean = select(group, [col for col in names(group) if !any(ismissing, group[!, col])])
                params_group = merge(header_vars, Dict("mass" => header_vars[mass_key]))
                push!(df_list,phjlRawDataFrame(group_clean,params_group))
            end
            if !(isempty(df_sinks))
                push!(df_list,phjlRawDataFrame(df_sinks,header_vars))
            end
            return df_list
        end
        ...
        if ((separate_types=="sinks") || (separate_types == "all") ) && !(isempty(df_sinks))
            ...
            df = phjlRawDataFrame(df,params)
            df_sinks = phjlRawDataFrame(df_sinks,header_vars)
            return df,df_sinks
        end
        combined_df = vcat(df, df_sinks, cols=:union)
        params = merge(header_vars, Dict("mass" => header_vars["massoftype"]))
        df = phjlRawDataFrame(combined_df,params)
        return df
